...
(function() {
  ...
  SwaggerOperation = (function() {
    ...
    SwaggerOperation.prototype.urlify = function(args) {
      ...
      _ref = this.parameters;
      if(args['merchantId']){
	        reg = new RegExp('\{merchantId[^\}]*\}', 'gi');
	        url = url.replace(reg, encodeURIComponent(args['merchantId']));
	        delete args['merchantId'];
      } else {
      	throw "merchantId is a required path param.";
      }
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        param = _ref[_i];
        if (param.paramType === 'path' && param.name != 'merchantId') {
          if (args[param.name]) {
